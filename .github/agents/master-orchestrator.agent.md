...
| **Scribe** | Both (journal in `cedibites_api/`) | Atomic decision ledger — locked decisions, rejections, deferrals, open questions, cross-repo impacts. One file: `docs/JOURNAL.md`. Silent on no-op turns. | After any turn that locked a decision, explicitly rejected an option, deferred work, surfaced an open question, or confirmed a cross-repo impact |
...
| Scribe | `.github/agents/scribe.agent.md` | `.github/agents/scribe.agent.md` (journal at `docs/JOURNAL.md`) |
...
- **Decision Journal (Scribe)**: `cedibites_api/docs/JOURNAL.md` — read this at first activation to understand all locked decisions, rejections, and open questions before planning any task
