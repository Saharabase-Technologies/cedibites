...
const ORDER_SOURCES = [
    { name: 'Online',    count: 48,  pct: 32, avgValue: 82  },
    { name: 'WhatsApp',  count: 36,  pct: 24, avgValue: 76  },
    { name: 'Phone',     count: 28,  pct: 18, avgValue: 64  },
    { name: 'POS',       count: 22,  pct: 14, avgValue: 55  },
    { name: 'Instagram', count: 11,  pct: 7,  avgValue: 91  },
    { name: 'Facebook',  count: 7,   pct: 5,  avgValue: 88  },
];
...
const TOP_ITEMS = [
    ...
    { name: 'Pineapple Juice',         units: 26,  rev: 520,  trend: -2  },
    { name: 'Combo - Jollof + Drink',  units: 21,  rev: 2310, trend: +9  },
];

const BOTTOM_ITEMS = [
    { name: 'Fried Yam (Large)',   units: 3, rev: 75  },
    { name: 'Kenkey & Fish',       units: 5, rev: 175 },
    { name: 'Kokonte & Egusi',     units: 6, rev: 210 },
];
...
const BRANCH_STATS = [
    { name: 'East Legon', rev: 14480, orders: 189, avg: 76.6, fulfilment: 93, cancelled: 4 },
    { name: 'Osu',        rev: 12700, orders: 166, avg: 76.5, fulfilment: 91, cancelled: 5 },
    { name: 'Spintex',    rev: 5210,  orders: 97,  avg: 53.7, fulfilment: 88, cancelled: 7 },
];
...
// ─── Revenue chart (multi-branch) ────────────────────────────────────────────

function RevenueChart() {
    const [stacked, setStacked] = useState(true);
    const totals = DAYS.map((_, i) => BRANCHES.reduce((s, b) => s + DAILY_REVENUE[b][i], 0));
    const maxVal = stacked ? Math.max(...totals) : Math.max(...Object.values(DAILY_REVENUE).flat());

    return (
        <Card>
            <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                <SectionTitle title="Daily Revenue — All Branches" sub="7-day view" />
                <div className="flex gap-1.5">
                    {['Stacked', 'Grouped'].map(m => (
                        <button key={m} type="button" onClick={() => setStacked(m === 'Stacked')}
                            className={`px-2.5 py-1.5 rounded-lg text-[11px] font-semibold font-body transition-all cursor-pointer ${(m === 'Stacked') === stacked ? 'bg-primary text-white' : 'bg-neutral-light text-neutral-gray hover:text-text-dark'}`}>
                            {m}
                        </button>
                    ))}
                </div>
            ...
            <div className="flex items-end gap-2 h-32">
                {DAYS.map((day, di) => (
                    <div key={day} className="flex-1 flex flex-col items-center gap-1">
                        <div className={`w-full flex ${stacked ? 'flex-col-reverse' : 'flex-row'} items-end gap-px`} style={{ height: 112 }}>
                            {BRANCHES.map((b, bi) => {
                                const val = DAILY_REVENUE[b][di];
                                const h = Math.round((val / maxVal) * 112);
                                return (
                                    <div key={b} className="rounded-sm"
                                        style={{ height: stacked ? h : h, width: stacked ? '100%' : `${100 / BRANCHES.length}%`, background: BRANCH_COLORS[bi], opacity: 0.85, transition: 'height 0.3s ease', flexShrink: 0 }} />
                                );
                            })}
                        </div>
                        ...
                        {ORDER_SOURCES.map((src, i) => {
                            ...
                            const seg = (
                                <circle key={src.name} cx="50" cy="50" r="32" fill="none"
                                    stroke={SOURCE_COLORS[i]} strokeWidth="14"
...
function TopItemsCard({ items, title }: { items: typeof TOP_ITEMS; title: string }) {
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                                ...
                                {'trend' in item && (
                                    <div className="flex items-center gap-0.5">
                                        {(item as typeof TOP_ITEMS[0]).trend > 0
                                            ? <ArrowUpIcon size={10} className="text-secondary" />
                                            : <ArrowDownIcon size={10} className="text-error" />
                                        }
                                        <span className={`text-[10px] font-body ${(item as typeof TOP_ITEMS[0]).trend > 0 ? 'text-secondary' : 'text-error'}`}>
                                            {Math.abs((item as typeof TOP_ITEMS[0]).trend)}%
                                        </span>
                                    </div>
                                )}
                            </div>
...
function BranchPerformanceTable() {
    const maxRev = Math.max(...BRANCH_STATS.map(b => b.rev));
    return (
        <Card>
                ...
                {BRANCH_STATS.map((b, i) => (
                    <div key={b.name}>
                        ...
                        <div className="h-2 bg-neutral-gray/15 rounded-full overflow-hidden">
                            <div className="h-full rounded-full" style={{ width: `${(b.rev / maxRev) * 100}%`, background: BRANCH_COLORS[i], transition: 'width 0.4s ease' }} />
                        </div>
                    </div>
                ))}
            </div>
            ...
            <Card>
                ...
                <div className="flex flex-col gap-0">
                    {TOP_CUSTOMERS.map((c, i) => (
                        <div key={c.name} className={`flex items-center gap-3 py-2.5 ${i < TOP_CUSTOMERS.length - 1 ? 'border-b border-[#f0e8d8]' : ''}`}>
    ...
    return (
        <div className="px-4 md:px-8 py-6 max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-5">
                <KpiCard icon={CurrencyCircleDollarIcon} label="Revenue"        value="GHS 32,390" trend={+14} accent />
                <KpiCard icon={ReceiptIcon}              label="Orders"         value="452"         trend={+8}        />
                <KpiCard icon={TrendUpIcon}              label="Avg. Order"     value="GHS 71.66"   trend={-2}        />
                <KpiCard icon={UsersIcon}                label="New Customers"  value="38"          trend={+22}       />
                <KpiCard icon={CheckCircleIcon}          label="Fulfilment"     value="91%"         trend={+3}        />
                <KpiCard icon={XCircleIcon}              label="Cancellations"  value="5.3%"        trend={-1}        />
            </div>
            ...
            {/* Top items */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                <TopItemsCard items={TOP_ITEMS.slice(0, 5)} title="Top 5 Items by Revenue" />
                <div className="flex flex-col gap-3">
                    <TopItemsCard items={BOTTOM_ITEMS.map(i => ({ ...i, trend: -15 }))} title="Slow Movers (Last 7 Days)" />
                </div>
            </div>
