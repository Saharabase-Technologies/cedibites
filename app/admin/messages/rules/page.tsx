                ...
                <div className="flex justify-end gap-2">
                    ...
                    {r.is_active && (
                        <button
                            ...
                            disabled={busy === r.id}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-rose-50 border border-rose-200 text-xs font-body text-rose-700 hover:border-rose-300 transition-colors disabled:opacity-50 cursor-pointer"
                        >
                            <PowerIcon size={14} />
                            Switch off
                        </button>
...
function DryRunDialog({
    ...
    return (
        <InventoryModal isOpen onClose={onClose} title={rule.name} size="lg">
            ...
            {noisy && (
                <p className="mt-4 rounded-xl bg-amber-50 border border-amber-200 p-3 font-body text-xs text-text-dark">
...
function Stat({ label, value, emphasis }: { label: string; value: number; emphasis?: boolean }) {
    return (
        <div className="rounded-xl bg-neutral-light border border-[#e3ddd0] p-3">
            ...
            <p
                className={`font-body text-lg font-semibold ${emphasis ? 'text-amber-700' : 'text-text-dark'}`}
            >
                {value}
            </p>
        </div>
